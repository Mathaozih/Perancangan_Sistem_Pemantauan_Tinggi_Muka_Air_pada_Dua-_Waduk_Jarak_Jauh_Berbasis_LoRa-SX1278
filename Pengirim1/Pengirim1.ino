/*
 * ============================================
 * PROGRAM PENGIRIM 1 (TX1) [PERBAIKAN HY-SRF05 & TYPO]
 * Node ID: 1
 * Logika sensor diperbarui untuk memfilter noise HY-SRF05.
 * ============================================
 */
...
// === PENGATURAN GLOBAL ===
const int NODE_ID = 1; // <-- ID PENTING
...
void setup() {
  ...
  lcd.init();
  lcd.backlight();
  lcd.print("Pengirim TX1"); 
  lcd.setCursor(0, 1);
  lcd.print("Node 1: Mulai...");

  Serial.println("Mulai LoRa TX1...");
  ...
  Serial.println("TX1 (Node 1) Siap.");
  delay(1000);
}
...
void loop() {
  ...
  if (currentMillis - previousMillis >= interval) {
    ...
    // ==============================================================
    // 1. BACA SENSOR (PERBAIKAN UNTUK HY-SRF05)
    // ==============================================================
    long durasi;
    ...
    // PERBAIKAN BARU:
    // Jika durasi = 0 (timeout) ATAU durasi < 150 (noise HY-SRF05)
    // anggap sebagai tidak ada pantulan / tangki kosong.
    if (durasi < 150) { 
      jarak = TANGKI_TINGGI + 1.0; // Set jarak lebih besar dari tinggi tangki
    } else {
      // Jika durasi valid (di atas 150), hitung seperti biasa
    ...
    // ==============================================================
    //  BARIS INI TELAH DIPERBAIKI (dataKdim -> dataKirim)
    // ==============================================================
    Serial.printf("TX1 Kirim: Node %d, Persen: %.1f, Status: %d\n", dataKirim.nodeId, dataKirim.percentage, dataKirim.status);
  }
}